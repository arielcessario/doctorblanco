module.exports = {
    "/server/api.php*": {
        target: "http://localhost:80/doctorblanco/src/app/server/api.php",
        secure: false,
        logLevel: "debug",
        changeOrigin: true,
        // headers: {
        //     "Authorization": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..iPqn-spv7iCH2T1MBtQbnDFEGVo48BQrD8SBbkTRqzU"
        // },
        onProxyRes: proxyRes => {
            let key = 'www-authenticate';
            proxyRes.headers[key] = proxyRes.headers[key] &&
                proxyRes.headers[key].split(',');
            // console.log(proxyRes.headers);
        }
    },"../images/*": {
        target: "http://localhost:80/doctorblanco/src/app/images",
        secure: false,
        logLevel: "debug",
        changeOrigin: true,

    },"/server/upload.php*": {
        target: "http://localhost:80/doctorblanco/src/app/server/upload/upload.php",
        secure: false,
        logLevel: "debug",
        changeOrigin: true,
        // headers: {
        //     "Authorization": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..iPqn-spv7iCH2T1MBtQbnDFEGVo48BQrD8SBbkTRqzU"
        // },
        onProxyRes: proxyRes => {
            let key = 'www-authenticate';
            proxyRes.headers[key] = proxyRes.headers[key] &&
                proxyRes.headers[key].split(',');
            // console.log(proxyRes.headers);
        }
    }
};